...
protected:
    void SetUp() override
    {
        ...
        kernel =
            std::make_shared<RealtimeKernel>(*timer, *logger, "test-kernel");
...
TEST_F(RealtimeKernelTest, PeriodicTasksRunBeforeIdleTasks)
{
    ...
    auto periodic1 = kernel->add_periodic(TaskType::SOFT_REALTIME,
        "periodic-100ms", 100ms, [&execution_order](BaseTask&) {
            execution_order.push_back("periodic1");
            return TaskStatus::TASK_OK;
        });
    periodic1->enable();

    auto periodic2 = kernel->add_periodic(TaskType::SOFT_REALTIME,
        "periodic-200ms", 200ms, [&execution_order](BaseTask&) {
            execution_order.push_back("periodic2");
            return TaskStatus::TASK_OK;
        });
    periodic2->enable();

    auto periodic3 = kernel->add_periodic(TaskType::HARD_REALTIME,
        "periodic-50ms", 50ms, [&execution_order](BaseTask&) {
    ...
    // Add 3 idle tasks
    auto idle1 = kernel->add_idle_task("idle1", [&execution_order](BaseTask&) {
        execution_order.push_back("idle1");
        return TaskStatus::TASK_OK;
    });

    auto idle2 = kernel->add_idle_task("idle2", [&execution_order](BaseTask&) {
        execution_order.push_back("idle2");
        return TaskStatus::TASK_OK;
    });

    auto idle3 = kernel->add_idle_task("idle3", [&execution_order](BaseTask&) {
    ...
    bool has_periodic1 =
        std::find(execution_order.begin(), execution_order.end(),
            "periodic1") != execution_order.end();
    bool has_periodic2 =
        std::find(execution_order.begin(), execution_order.end(),
            "periodic2") != execution_order.end();
    bool has_periodic3 =
        std::find(execution_order.begin(), execution_order.end(),
            "periodic3") != execution_order.end();
    ...
    auto first_idle_pos =
        std::find_if(execution_order.begin(), execution_order.end(),
            [](const std::string& name) { return name.find("idle") == 0; });

    // If any idle tasks ran, verify all periodic tasks before them are actually
    // periodic
    if (first_idle_pos != execution_order.end())
    {
        for (auto it = execution_order.begin(); it != first_idle_pos; ++it)
        {
            ...
            if (it->find("idle") == 0)
            {
                FAIL() << "Found idle task '" << *it
                       << "' before first idle task position";
            }
        }
    }
...
TEST_F(RealtimeKernelTest, PeriodicTasksScheduledByDeadline)
{
    ...
    auto periodic_fast = kernel->add_periodic(
        TaskType::HARD_REALTIME, "fast-10ms", 10ms, [&](BaseTask&) {
    ...
    auto periodic_medium = kernel->add_periodic(
        TaskType::SOFT_REALTIME, "medium-50ms", 50ms, [&](BaseTask&) {
    ...
    auto periodic_slow = kernel->add_periodic(
        TaskType::SOFT_REALTIME, "slow-100ms", 100ms, [&](BaseTask&) {
...
TEST_F(RealtimeKernelTest, IdleTasksRunWhenNoPeriodicTasksReady)
{
    ...
    auto periodic = kernel->add_periodic(
        TaskType::SOFT_REALTIME, "periodic-1s", 1s, [&](BaseTask&) {
    ...
    // Add idle tasks
    auto idle1 = kernel->add_idle_task("idle1", [&](BaseTask&) {
        execution_order.push_back("idle1");
        return TaskStatus::TASK_OK;
    });

    auto idle2 = kernel->add_idle_task("idle2", [&](BaseTask&) {
        execution_order.push_back("idle2");
        return TaskStatus::TASK_OK;
    });
    ...
    int idle_count =
        std::count_if(execution_order.begin(), execution_order.end(),
            [](const std::string& name) { return name.find("idle") == 0; });

    EXPECT_GT(idle_count, 0)
        << "Idle tasks should have run when no periodic tasks were ready";
}
